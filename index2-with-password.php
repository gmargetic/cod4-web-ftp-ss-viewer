...
$cfg['auth']['pass'] = "mySuperCoolPw";       // access password
...
if(!empty($_POST['auth']) AND $_POST['auth'] == "true") {
	if($_POST['password'] == $cfg['auth']['pass']) {
		$_SESSION['authviewer'] = md5($_POST['password']);
	} else {
	...
	function ftp_get_filelist($con, $path){
		...
		if(count($contents)){
			foreach($contents as $line){
				preg_match("#([0-9]+\-[0-9]+\-[0-9]+)(\s+)([0-9]+\:[0-9]+(AM|PM))(\s+)([0-9]+)(\s+)(.*\.jpg)#si", $line, $out);

				if(@$out[3] != 1 && (@$out[18] == "." || @$out[18] == "..")){
					// do nothing for now
				} else {
					$a++;
					@$files[$a]['date_modified']    = $out[1]." ".$out[3];
  ...
  <body>
	...
	<?php } else { ?>
		<div class="row">
			<div class="col-md-6">
				<form method="post">
					<input type="hidden" name="auth" value="true" required>
